...
var {
  numberOfDaysSince,
  groupByKey,
} = require("../bin/general-helper-functions");
      ...
      BookingModel.aggregate([
        {
          $match: {
            $and: [
              ...
              { $or: [{ paid: true }, { bookingType: "request_booking" }] },
            ],
          },
        },
        { $sort: { creationDate: 1 } },
...
router.get("/freelancer/:this_user", async function (req, res, next) {
  ...
  try {
    ...
    if (freelancerUser.is_subscribed) {
      ...
      let numberOfDaysSinceJoining = numberOfDaysSince(
        freelancerUser.joiningDate
      );
...
router.put(
  "/client/update",
  ensureAuthentication,
  multerUserProfilePicture,
  async function (req, res, next) {
    try {
      let clientInformationChecker = new UpdateProfileChecker(
        req.user,
        req.body,
        req.file,
        "client"
      ...
      let updateInfos =
        await clientInformationChecker.checkLoginStrategyAndReturnInformation();
...
router.put(
  ...
  async function (req, res, next) {
    try {
      let freelancerInformationChecker = new UpdateProfileChecker(
        req.user,
        req.body,
        req.file,
        "freelancer"
      ...
      let updateInfos =
        await freelancerInformationChecker.checkLoginStrategyAndReturnInformation();
  ...
  async function (req, res, next) {
    ...
    UserModel.findOne({ email: emailDecode(userUUID) })
      .then((user) => {
        ...
        else flash_message = "Switch to Freelancer was successful";
        ...
        user.save((err) => {
          ...
          if (!previousURL.includes("receiverKey")) {
            if (previousURL === `${domainName}/`) res.redirect(`back`);
            else res.redirect(`/account/freelancer/${userUUID}`);
          } else res.redirect(`/account/freelancer/${userUUID}`);
        });
      })
      .catch((error) => {
