...
function showServicesAndPrices(elements, htmlcontainer) {
  elements.forEach((singleServiceAndPrice) => {
    ...
    if (priceOfService >= minimumPriceOfService) {
      ...
      singleServicePriceClass.classList.add(
        "freelancer-serviceAndPrice-profile"
      );
      $(singleServicePriceClass).append("<div></div>");
      ...
      if (servicePackage.length > 0) {
        ...
        $(singleServicePriceClass).append(
          '<div class="freelance-package-offer" style="display: none"></div>'
        );

        $(singleServicePriceClass.lastChild).append(
          "<h4>Package Offer:</h4><section><ul></ul></section>"
        );
        ...
        servicePackage.forEach((singlePackage) => {
          ...
          $(singleServicePriceClass.lastChild.lastChild.firstChild).append(
            onePackageList
          );
        });
      }
  ...
  $(document).on("click", containerHtml, function (event) {
    ...
    if (item.classList[0] === buttonHtml) {
      ...
      if (buttonHtml === "delete-aServiceAndPrice-btn") {
            ...
            [
              {
                label: "updateError",
                message:
                  "Service cannot be empty! Please add more services to delete this.",
              },
            ],
          ];
        ...
        buttonHtml === "update-error-delete-btn" ||
        buttonHtml === "update-delete-btn"
      ) {
        container.remove();
      } else {
...
function createNewRoom(receiverData, sourceImage) {
  ...
  roomContainerDetails_h5Name.innerText =
...
function createNewConversationContainer(receiverData, sourceImage) {
  ...
  conversationsContainer_h4Name.innerText =
    receiverData.name + " " + receiverData.surname;
  ...
  $(conversationsContainer_sendFormInput).attr(
    "placeholder",
    "Write a message..."
  );
...
function roomConversationsNavigation(
  roomToShow,
  roomsContainerHtml,
  loggedInUser,
  receiver
        ...
        success: function (data) {
          data.forEach((singleMessage) => {
            ...
            if (loggedInUser === singleMessage.sender) {
              createMessageHTML(
                singleMessage,
                "user-single-send-message",
                currentRoomContainer.childNodes[1]
              );
            } else {
              createMessageHTML(
                singleMessage,
                "user-single-receive-message",
                currentRoomContainer.childNodes[1]
              );
            }
          });
...
function createBookingTopHTML(bookingData, projectHTML, projectDetailsHTML) {
  ...
  if (bookingData.status === "booking ongoing") {
    ...
    bookingData.status === "awaiting resolution" ||
  ...
  return [
    projectContainer,
    createBookingToggleDetails(bookingData, projectDetailsHTML),
...
function createBookingToggleDetails(bookingData, projectDetailsHTML) {
  ...
  let projectDetailsDescription =
    `<div>` +
    `<h4>Creation Date: <span>${new Date(
      bookingData.creationDate
    ).toLocaleString(locale)}</span></h4>` +
    `<div class="bookingDetails-showOnMobileResponsive">` +
    `<h4>Client Name: <span>${bookingData.customer.name}</span></h4>` +
    `<h4>Due Date: <span>${new Date(bookingData.dueDateTime).toLocaleString(
      locale
    )}</span></h4>` +
    `</div>` +
...
function createBookingFinalMessages(bookingData) {
  ...
  contactInfos.innerText =
    "(contact us on kingshire.team@gmail.com for enquiries)";
  ...
  projectCompletionRejected.classList.add(
    "freelancer-project-completion-rejection"
  );
  let messageInfos_failure = document.createElement("p");
  messageInfos_failure.innerHTML =
    `${bookingData.customer.name} has decided to ` +
    "<span> cancel </span>" +
...
function createBookingModificationHTML(
  originalBookingData,
  modifyBookingData,
  detailsDIV
) {
  let projectDetailsDescriptionOriginalHTML =
    `<div class="freelancer-side-originalBooking">` +
    `<div>` +
    `<h4>Original</h4>` +
    `</div>` +
    `<div>` +
    ...
    `<span style="font-weight: normal">${new Date(
      originalBookingData.creationDate
    ).toLocaleString(locale)}</span>` +
    `</h4>` +
    `<div class="bookingDetails-showOnMobileResponsive">` +
    `<h4>Client Name: <span>${originalBookingData.customer.name}</span></h4>` +
    `<h4>Due Date: <span>${new Date(
      originalBookingData.dueDateTime
    ).toLocaleString(locale)}</span></h4>` +
    `</div>` +
    `<div class="freelancer-project-details-descritpion">` +
    `<h4>Description: </h4>` +
  ...
  let projectDetailsDescriptionProposedHTML =
    `<div class="freelancer-side-BookingChangesProposed" style='color: #8d2874; font-style: italic'>` +
    `<div>` +
    `<h4>Your Proposal</h4>` +
    `</div>` +
    `<div>` +
    ...
    `<span style="font-weight: normal">${new Date(
      modifyBookingData.newProposedDueDate
    ).toLocaleString(locale)}</span>` +
    `</h4>` +
    `<div class="bookingDetails-showOnMobileResponsive">` +
    `<h4>Client Name: <span>${originalBookingData.customer.name}</span></h4>` +
...
function bookingModificationClientSide(
  originalBookingData,
  modifyBookingData,
  insertionDIV
) {
  ...
  let originalBookingHTML =
    `<div class="client-side-originalBooking">` +
    `<div><h4>Original Booking</h4></div><div><h4>Creation Date: ` +
    `${new Date(originalBookingData.creationDate).toLocaleString(
      locale
    )}</h4>` +
    ...
    `${new Date(originalBookingData.dueDateTime).toLocaleString(
      locale
    )}</span></h4>` +
    `</div>` +
  ...
  let modifiedBookingHTML =
    `<div class="client-side-modificationBooking">` +
    `<div><h4>New Proposal</h4></div><div><h4>Proposed Due Date: ` +
    `${new Date(modifyBookingData.newProposedDueDate).toLocaleString(
      locale
    )}</h4>` +
...
function moveProjectBooking(
  ...
  $(nextDueProjectHTML.firstChild)
    .children()
    .clone()
    .appendTo(projectTopContainerHTML);
  ...
  $(nextDueProjectHTML.childNodes[1])
    .children()
    .clone()
    .appendTo(projectDetailsContainerHTML);

  return [
    projectTopContainerHTML,
    projectDetailsContainerHTML,
    createBookingIdInput(bookingID),
  ];
}
...
export {
  ...
  deleteItem,
  keyBoardAction,
  ajaxFormMessage_generator,
  disbaleButton,
  enableButton,
  createNewRoom,
  createNewConversationContainer,
  roomConversationsNavigation,
  createMessageHTML,
  createBookingHTML,
  createBookingTopHTML,
  createBookingModificationHTML,
  bookingModificationClientSide,
  moveProjectBooking,
};
