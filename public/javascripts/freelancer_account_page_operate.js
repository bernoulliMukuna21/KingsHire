...
let freelancerProfileSections = $(".freelancer_account_main_side")[0]
  .childNodes;
  ...
  // START FROM HERE
    ...
    if (inputHtmlName === "service") {
      let placeholderValue =
        "Enter full package " +
        serviceOrSkillValue +
        " for for this price?(<5)";
      let servicePackageHTM =
        `<div class="show-service-package">` +
...
$(document).on("click", ".show-service-package", function (e) {
  if (
    e.target.className === "fas fa-plus-square" ||
    ...
    else newPackageHTML = e.target.previousSibling;
    ...
    if (packageText.trim().length > 0) {
      ...
      if (serviceDetails.childNodes.length === 2) {
        ...
        servicePackageContainer.classList.add(
          "freelancer-service-package-update"
        );
      ...
      let hideServicePackageValue = JSON.parse(
        hideServicePackage.lastChild.value
      );
      hideServicePackageValue.freelancerPackage.push(newPackageHTML.value);
      hideServicePackage.lastChild.value = JSON.stringify(
        hideServicePackageValue
      );

      $(servicePackageContainer).append(
        `<li><p>- ${packageText}</p><p class="freelance-delete-aPackage">x</p></li>`
      );
...
function projectCompletionShow(event, data) {
  ...
  $(
    bookingCompletionHTML.firstChild.firstChild.childNodes[1].firstChild
  ...
  if (buttonHTML.className === "freelancer-booking-finish-button") {
    ...
    let modificationDateTime =
      modificationForm.childNodes[0].firstChild.childNodes;
    ...
    // price
    $(modificationForm.childNodes[2].childNodes[1].childNodes[1]).val(
      data.modifyPrice
    );
    $(modificationForm.childNodes[2].childNodes[1].childNodes[1]).attr(
      "placeholder",
      data.modifyPrice
    );
    ...
    acceptanceForm.childNodes[1].childNodes[1].innerText =
      data.descriptionToAccept;
